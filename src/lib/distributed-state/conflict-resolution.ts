...
export function resolveConflict<T>(input: ConflictInput<T>): { value: T; needsManual: boolean } {
  ...
  switch (strategy) {
    case "first-write-wins":
      return { value: input.local.updatedAt <= input.remote.updatedAt ? input.local.value : input.remote.value, needsManual: false };
    case "merge":
      return { value: input.merge ? input.merge(input.local.value, input.remote.value) : input.remote.value, needsManual: false };
    case "manual":
