...
export class OutreachAgent extends BaseAgent {
  ...
  readonly capabilities: AgentCapability[] = [
    ...
    },
    {
      ...
      version: "1.0",
      description: "Routes messages to appropriate channels",
    },
  ];
  ...
  async generateMessage(
    lead: Lead,
    channel: "email" | "in_app" | "webhook" = "email",
  ...
  private generateSubject(lead: Lead): string {
    ...
    return template.replace("{company}", lead.company ?? "").replace("{firstName}", lead.firstName);
  }
